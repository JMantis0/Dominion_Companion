...
const HistoryModal: FunctionComponent = () => {
  ...
  const ods = useSelector((state: RootState) => state.options.opponentDecks);
  ...
  return (
    <React.Fragment>
      <div
        id="modal"
        className="backdrop-blur-sm bg-[#232122] grid grid-cols-12 h-[80vh]"
      >
        <CloseModalButton />
        <div className={"col-span-8 grid grid-cols-12"}>
          <div className="col-span-12">
            <h4 className="text-center">
              {pd.playerName} vs. {ods[0].playerName}
            </h4>
          ...
          {ods.map((od, idx) => {
            return (
              <div key={idx} className="col-span-6 h-[80vh] m-auto">
