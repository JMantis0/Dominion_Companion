...
describe("saveGameData function", () => {
  ...
  it("should set the game keys to include the new title, then set the new saved game to chrome storage, then dispatch all the saved games in chromes storage to redux", async () => {
    // Arrange Class fields
    DOMObserver.playerName = "Player";
    DOMObserver.opponentName = "Opponent";
