...
import { sendTurnOnRequest } from "../../src/utils/utils";
describe("sendTurnOnRequest", () => {
  ...
  it(
    "should try to get the active tab, and if it's not undefined, send a request to the content script" +
      " and dispatch the given SetStateAction with the action 'ON'",
    async () => {
      // Arrange
      ...
      await sendTurnOnRequest(setStateMock);
      ...
      expect(setStateMock).toBeCalledWith("ON");
    }
  );
